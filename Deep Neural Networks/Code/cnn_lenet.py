...
def conv_net(params, layers, data, labels, doBackProp):
  ...
  if doBackProp:
    ...
    if layers[2]['iteration'] == 29:
      print output[1]['data'].shape
      channel = output[1]['data'][:,0].reshape(28,28)
      # for ch in range(channel.shape[2]):
      #   array2 = channel[:,:,ch]
      #   plt.subplot(4, 5, ch + 1)
      # plt.imshow(channel, cmap='gray')      # <- I changed this line
      # plt.savefig('Orginal.PNG')
      # plt.show()
      
      channel = output[2]['data'][:,0].reshape(24,24,20)
      for ch in range(channel.shape[2]):
        array2 = channel[:,:,ch]
        plt.subplot(4, 5, ch + 1)
        plt.imshow(array2, cmap='gray')      # <- I changed this line
      plt.savefig('Convolution-30.PNG')
      plt.show()

      # channel = output[3]['data'][:,0].reshape(12,12,20)
      # for ch in range(channel.shape[2]):
      #   array2 = channel[:,:,ch]
      #   plt.subplot(4, 5, ch + 1)
      #   plt.imshow(array2, cmap='gray')      # <- I changed this line
...
def pooling_layer_forward(input, layer):
  ...
  for n in range(batch_size):
    input_i['data'] = input['data'][:, n]
    col = im2col_conv(input_i, layer, h_out, w_out)
    # print col.shape
    jump = 0
    final = np.zeros(shape=c*col.shape[1])
    for i in range(col.shape[1]):
      max_mat = np.max(col[:,i].reshape(k*k,c), axis=0)
      final[i+jump:i+jump+c] = max_mat
      jump += c - 1
    output['data'][:,n] = final
  # implementation ends
...
def relu_forward(input, layer):
  ...
  for i in range(output['data'].shape[0]):
    for j in range(output['data'].shape[1]):
      if input['data'][i][j] < 0:
        output['data'][i][j] = 0
      else:
...
def inner_product_forward(input, layer, param):
  ...
  for n in range(batch_size):
    output['data'][:,n] = input['data'][:,n].dot(param['w'][:,]) + param['b']
  # implementation ends
